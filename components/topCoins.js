...
function TopCoins ({rank, name, image, price, percent}){
  ...
  const { symbol } = useContext(CryptoState)
    ...
    return(
        <div className="flex space-x-3">
            {/* <Link href={coinrankingUrl} passHref> */}
            <div className="flex flex-col my-3 w-28 sm:w-36 sm:h-36 h-32 pt-4 rounded-2xl bg-white shadow-2xl text-center cursor-pointer">
                ...
                {/* <span>{prices}</span> */}
                <div className="flex justify-center items-center font-bold text-xs sm:text-xl" style={{ color:getStatusText(percent)}}>
                    <ArrowDropDownIcon />
                    <h1>{percents}</h1>
                </div>
              </div>
